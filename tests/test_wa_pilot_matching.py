"""WA-17: WA pilot classify+match — federal gate, posture advisory, review label."""
...
def test_wa_federal_no_tier_mismatch_on_federal_required() -> None:
...
def test_wa_grant_posture_advisory_not_hard_filter() -> None:
    block = run_wa_pilot_classify_match_block(
        require_fixtures=True, grants=_SYNTH_GRANTS
    )
    compact_profiles = [
...
def test_wa_all_matches_needs_operator_review() -> None:
    block = run_wa_pilot_classify_match_block(
        require_fixtures=True, grants=_SYNTH_GRANTS
    )
...
def test_wa_grant_posture_advisory_contract() -> None:

...
def test_production_write_blocked_without_approval() -> None:
    clear_local_dev_metadata_store_for_tests()
    result = production_metadata_write_attempt(organization_profile_id="org_a")
    assert result["status"] == "blocked"
    assert "owner_approval_absent" in result["reasons"]
    assert result["production_storage_claimed"] is False
    events = get_metadata_audit_events()
...
def test_local_dev_allowed_and_cross_org_denied() -> None:
    ...
    w = local_dev_metadata_write(
        organization_profile_id="org_a", original_filename="a.pdf"
    )
    assert w["status"] == "ok"
    eid = w["record"]["evidence_id"]
    ok = local_dev_metadata_read(evidence_id=eid, requesting_org_id="org_a")
    assert ok["status"] == "ok"
    denied = local_dev_metadata_read(evidence_id=eid, requesting_org_id="org_b")
    assert denied["status"] == "denied_cross_org"
    assert any(
        e["event"] == "local_dev_metadata_cross_org_denied"
        for e in get_metadata_audit_events()
    )

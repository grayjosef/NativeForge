"""WA-18: unknown program areas stay discoverable and force operator review."""
...
def test_wa_program_fit_unknown_profile_stays_unknown() -> None:
    ...
    if not unknown_profiles:
        pytest.skip("no WA profile with unknown program_areas in fixtures")
...
def test_wa_unknown_profiles_remain_in_match_set() -> None:
    block = run_wa_pilot_classify_match_block(
        require_fixtures=True, grants=_SYNTH_GRANTS
    )

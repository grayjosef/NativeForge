...
from nativeforge.services.object_storage_signed_url_service import (
    ...
    clear_object_storage_audit_for_tests,
    generate_signed_download_url,
    generate_signed_upload_url,
    get_object_storage_audit_events,
    malware_scan_hook,
...
def test_object_keys_org_scoped_and_cross_org_denied() -> None:
    ...
    denied = generate_signed_download_url(
        organization_profile_id="org_b", object_key=key
    )
    assert denied["status"] == "denied_cross_org"
    archive = archive_or_delete_object(
        organization_profile_id="org_a", object_key=key, action="delete"
    )
    ...
    assert any(
        e["event"] == "signed_download_cross_org_denied"
        for e in get_object_storage_audit_events()
    )

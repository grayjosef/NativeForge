"""Sprint 70: explicit human authorization decision packet from human authorization request (stateless, no DB)."""
...
SERVICE_PATH = (
    ...
    / "nativeforge"
    / "services"
    / "active_source_activation_human_authorization_decision_packet_service.py"
)
...
def test_deterministic_output() -> None:
    r69 = _valid_sprint69_human_authorization_request_packet()
    a = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    b = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert a == b


def test_preview_no_execution_no_activation_guardrails() -> None:
    ...
    g = out["explicit_preview_only_no_execution_no_activation_guardrail"]
    assert "preview_only" in g
    assert "no_execution" in g
    assert "no_activation" in g


def test_explicit_authorization_decision_recorded_false_by_default() -> None:
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=_valid_sprint69_human_authorization_request_packet(),
    )
    assert out["explicit_authorization_decision_recorded"] is False
...
def test_explicit_authorization_decision_recorded_true_with_marker() -> None:
    ...
    assert out["explicit_authorization_decision_recorded"] is True


def test_future_activation_execution_plan_review_required_only_when_ready() -> None:
    ready_out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=_valid_sprint69_human_authorization_request_packet(),
    )
    assert ready_out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_READY
    assert ready_out["future_activation_execution_plan_review_required"] is True
...
def test_sprint69_compatibility_ready_path() -> None:
    r69 = _valid_sprint69_human_authorization_request_packet()
    assert r69["human_authorization_request_status"] == HUMAN_AUTH_REQUEST_READY
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_READY
    ref = out["human_authorization_request_packet_reference"]
...
def test_blocked_request_status_on_sprint69_packet() -> None:
    r69 = dict(_valid_sprint69_human_authorization_request_packet())
    r69["human_authorization_request_status"] = HUMAN_AUTH_REQUEST_BLOCKED
    r69["request_blockers"] = ["forced"]
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED
...
def test_actual_execution_indicator_blocks() -> None:
    r69 = dict(_valid_sprint69_human_authorization_request_packet())
    r69["actual_command_execution_count"] = 1
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED
...
def test_missing_no_activation_on_request_blocks() -> None:
    r69 = dict(_valid_sprint69_human_authorization_request_packet())
    del r69["no_activation"]
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED
...
def test_missing_readiness_reference_blocks() -> None:
    r69 = dict(_valid_sprint69_human_authorization_request_packet())
    del r69["authorization_readiness_packet_reference"]
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED
...
def test_sprint69_module_still_importable() -> None:
    m = importlib.import_module(
        "nativeforge.services.active_source_activation_human_authorization_request_packet_service"
    )
    assert callable(m.build_active_source_activation_human_authorization_request_packet)
...
def test_marker_on_blocked_packet_still_sets_explicit_recorded() -> None:
    ...
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED
    assert out["explicit_authorization_decision_recorded"] is True
    assert out["future_activation_execution_plan_review_required"] is False


def test_wrong_request_artifact_type_blocks() -> None:
    r69 = dict(_valid_sprint69_human_authorization_request_packet())
    r69["artifact_type"] = "wrong"
    out = build_active_source_activation_human_authorization_decision_packet(
        human_authorization_request_packet_artifact=r69,
    )
    assert out["human_authorization_decision_status"] == HUMAN_AUTH_DECISION_BLOCKED

...
ALSO needs live-fetch opt-in [*]     DOES need live-fetch opt-in
...
[*] Corrected in Gate 171. This table originally said a preflight does NOT
need the live-fetch opt-in. It does: `resolve_source_authority` reports
`governance_complete` only when all three decision kinds are signed, and the
warrant refuses on `governance_complete` whatever the warrant kind. Gate 171's
first preflight was refused with
`source_id_is_not_an_authorized_source:activated` and the sentence, not the
warrant, turned out to be wrong. The narrower distinction the table draws -
one path, GET only, authorizes nothing else - still holds.
...
# The RFC 9309 logic this gate wrote now lives in a service, because
# Gate 171 needed the same verdicts for two more hosts and a second
# parser would eventually disagree with this one. Re-exported here so
# importers of this module - rederive_gate163_robots_verdict.py - keep
# working, and so the promoted copy is what produced the stored 403's
# re-derived verdict.

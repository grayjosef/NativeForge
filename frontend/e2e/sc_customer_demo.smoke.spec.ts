...
test.describe("SC customer demo Playwright smoke", () => {
  test("renders required Monday customer surfaces", async ({ page }) => {
    ...
    await expect(page.getByTestId("sc-demo-gate33-source-probes")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate33-source-probes-flags"),
    ).toContainText("live=false");
    await expect(page.getByTestId("sc-demo-gate33-healthchecks")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate33-healthchecks-flags"),
    ).toContainText("alert_sent=false");
    await expect(page.getByTestId("sc-demo-gate33-restore")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate33-restore-flags"),
    ).toContainText("prod_restore=false");
    await expect(page.getByTestId("sc-demo-gate33-runbooks")).toBeVisible();
    await expect(
    ...
    await expect(page.getByTestId("sc-demo-gate35-auth0")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate35-auth0-flags"),
    ).toContainText("login_live=false");
    await expect(page.getByTestId("sc-demo-gate35-storage")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate35-storage-flags"),
    ).toContainText("prod_storage=false");
    await expect(page.getByTestId("sc-demo-gate35-pentest")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate35-pentest-flags"),
    ).toContainText("pass=false");
    await expect(page.getByTestId("sc-demo-gate35-pilot")).toBeVisible();
    await expect(
      page.getByTestId("sc-demo-gate35-pilot-flags"),
    ).not.toContainText("CONTROLLED_CUSTOMER_GO");

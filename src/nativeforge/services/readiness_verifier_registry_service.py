...
VERIFIERS: tuple[dict[str, Any], ...] = (
    ...
    _verifier(
        ...
        note=(
            ...
            "nothing: a persistent organisational licence at $34,999 with the "
            "first twelve months of maintenance included, $6,999 a year "
            ...
            "docs/operations/570 carries other figures and says of itself "
            "that they are the operator's drafts recorded verbatim as drafts; "
            "the survey asserts none of THOSE reached the code. Where Tribes "
            "join together the thing being sold stops being one licence for "
            "one organisation, so an INTERTRIBAL_CONSORTIUM suite is priced "
            "on three named dimensions - isolation need, complexity and "
            "seats - and is QUOTED rather than computed. There is no "
            "arithmetic in that module that could produce a figure: a "
            "function multiplying three factors would return a wrong number, "
            "confidently, to a group of sovereign governments negotiating "
            "together, and the wrongness would be invisible because it would "
            "look like arithmetic rather than like a guess. A request that "
            "arrived carrying a price is refused, and a consortium that has "
            "not stated its isolation model is not quotable - "
            "ISOLATION_UNKNOWN is an outcome, not a default. "
            "ISOLATED_PER_MEMBER is the same tenant isolation Gate 179 "
            "proves, which is why that proof matters commercially. The "
            "load-"

...
SCHEMA_VERSION = "nf_campaign_block58_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block58_smoke")


def run_campaign_block58_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp58_pilot_master_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_controlled_pilot_master_demo_surface()
    fails.extend(controlled_pilot_master_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("controlled_pilot_master"):
        fails.append("bridge_missing_controlled_pilot_master")

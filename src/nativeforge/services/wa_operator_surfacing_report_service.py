"""WA operator surfacing report builder — review visibility over existing outputs.
...
SCHEMA_VERSION = "nf_wa_operator_surfacing_report_v1"
...
def build_wa_operator_review_items(
    *,
    grants: list[dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    """Sprint 021: WA review items from existing classify+match / readiness."""
    corpus = grants if grants is not None else _DEFAULT_GRANTS
    block = run_wa_pilot_classify_match_block(
        grants=corpus, allow_live_completeness_fetch=False
    )
    gaps = _gaps_for_state("WA", corpus)
...
def build_wa_operator_surfacing_report(
    *,
    grants: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    """Sprint 022–025: WA operator report + rollup summary."""
    corpus = grants if grants is not None else _DEFAULT_GRANTS
    items = build_wa_operator_review_items(grants=corpus)

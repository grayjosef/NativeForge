...
SCHEMA_VERSION = "nf_campaign_block43_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block43_smoke")


def run_campaign_block43_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp43_auth0_live_val_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    validation = run_auth0_live_validation()
    if validation.get("secret_value_printed"):
        fails.append("secret_printed")
    if validation.get("login_live_claimed"):
        fails.append("login_live")
    surface = build_auth0_live_validation_demo_surface()
    fails.extend(auth0_live_validation_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("auth0_live_validation"):
        fails.append("bridge_missing_auth0_live_validation")

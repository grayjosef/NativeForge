"""NM-0: load operator-provided NM pilot fixtures — no synthesis."""
...
SCHEMA_VERSION = "nf_nm_pilot_fixture_loader_v1"
PROFILES_SCHEMA = "nm_tribal_profiles_v1"
RESEARCH_PROFILES_FORMAT = "nm_tribal_profiles_research_v1"
...
EXPECTED_PROFILE_COUNT = 22

_NM_PILOT_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "nm_pilot"
PROFILES_PATH = _NM_PILOT_DIR / "nm_tribal_profiles.json"


class NMPilotFixtureError(FileNotFoundError):
...
def require_nm_pilot_fixtures() -> None:
    missing = [name for name, ok in fixtures_present().items() if not ok]
    if missing:
        raise NMPilotFixtureError(
            f"NM pilot fixtures missing in {_NM_PILOT_DIR}: {', '.join(missing)}. "
            "Place operator-sourced nm_tribal_profiles.json — "
...
def load_nm_tribal_profiles(*, require_files: bool = True) -> list[dict[str, Any]]:
    if require_files:
    ...
    if isinstance(raw, list):
        ...
        raise ValueError(
            f"nm_tribal_profiles.json: expected {PROFILES_SCHEMA!r} object or "
            f"research profile array — got {type(raw).__name__}"
        )
    if len(profiles) != EXPECTED_PROFILE_COUNT:
        raise ValueError(
            f"expected {EXPECTED_PROFILE_COUNT} NM tribal profiles, got {len(profiles)}"
        )
    return _json_safe(profiles)


def build_nm_pilot_fixture_contract() -> dict[str, Any]:
    present = fixtures_present()
    ready = all(present.values())
    profile_count = 0
    if ready:
        profile_count = len(load_nm_tribal_profiles(require_files=False))
    return _json_safe(
        {
            "schema_version": SCHEMA_VERSION,
            "fixtures_dir": str(_NM_PILOT_DIR),

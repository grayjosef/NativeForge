...
SCHEMA_VERSION = "nf_campaign_block59_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block59_smoke")


def run_campaign_block59_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp59_owner_unlock_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_owner_unlock_demo_surface()
    fails.extend(owner_unlock_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("owner_unlock_packet"):
        fails.append("bridge_missing_owner_unlock_packet")

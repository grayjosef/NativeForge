...
SCHEMA_VERSION = "nf_campaign_block60_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block60_smoke")


def run_campaign_block60_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp60_cutover_freeze_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_cutover_claim_freeze_demo_surface()
    fails.extend(cutover_claim_freeze_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("cutover_claim_freeze"):
        fails.append("bridge_missing_cutover_claim_freeze")

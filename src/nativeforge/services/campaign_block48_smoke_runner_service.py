...
SCHEMA_VERSION = "nf_campaign_block48_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block48_smoke")


def run_campaign_block48_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp48_storage_pilot_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_gate21_storage_pilot_demo_surface()
    fails.extend(gate21_storage_pilot_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("gate21_storage_pilot"):
        fails.append("bridge_missing_gate21_storage_pilot")
    ...
    result = {
        ...
        "final_controlled_pilot_status": surface.get("final_controlled_pilot_status"),

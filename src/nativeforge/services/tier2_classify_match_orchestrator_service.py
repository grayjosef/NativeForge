"""T2-3: classify + match Tier-2 state grants; all needs_operator_review."""
...
from nativeforge.services.tier2_state_corpus_persist_service import (
    load_mixed_tier123_corpus,
    load_tier2_state_corpus,
)

SCHEMA_VERSION = "nf_tier2_classify_match_orchestrator_v1"
...
def _apply_tier2_operator_review_gate(
    ...
    for match in classify_match.get("matches") or []:
        ...
        m["tier2_operator_review_required"] = True
        matches.append(m)
...
def run_tier2_classify_match_block(
    *,
    org_id: Any = None,
    ...
    if corpus is None:
        corpus = (
            load_mixed_tier123_corpus()
            if use_mixed_corpus
            else load_tier2_state_corpus()
        )
    raw_cm = classify_and_match_real_grants(grants=corpus)
    classify_match = _apply_tier2_operator_review_gate(raw_cm)
    queues = build_real_grant_workbench_queues(classify_match_result=classify_match)
    tier2_only = [
        g for g in corpus
        if g.get("tier") == 2 or str(g.get("grant_id", "")).startswith("ta2-")
    ]
    no_live_nofo = sum(1 for g in tier2_only if g.get("no_live_nofo"))
    real_fetch = sum(1 for g in tier2_only if g.get("real_fetch"))
    return _json_safe(
        {
            ...
            "tier2_grant_count": len(tier2_only),
            "tier2_real_fetch_count": real_fetch,
            "tier2_no_live_nofo_count": no_live_nofo,
            "tier2_grants": tier2_only,

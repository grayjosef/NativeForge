...
SCHEMA_VERSION = "nf_campaign_block72_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block72_smoke")


def run_campaign_block72_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp72_observability_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_observability_demo_surface()
    fails.extend(observability_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("observability"):
        fails.append("bridge_missing_observability")

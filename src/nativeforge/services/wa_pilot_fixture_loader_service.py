"""WA-0: load operator-provided WA pilot fixtures — no synthesis."""
...
SCHEMA_VERSION = "nf_wa_pilot_fixture_loader_v1"
PROFILES_SCHEMA = "wa_tribal_profiles_v1"
RESEARCH_PROFILES_FORMAT = "wa_tribal_profiles_research_v1"
...
_WA_PILOT_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "wa_pilot"
PROFILES_PATH = _WA_PILOT_DIR / "wa_tribal_profiles.json"


class WAPilotFixtureError(FileNotFoundError):
...
def require_wa_pilot_fixtures() -> None:
    missing = [name for name, ok in fixtures_present().items() if not ok]
    if missing:
        raise WAPilotFixtureError(
            f"WA pilot fixtures missing in {_WA_PILOT_DIR}: {', '.join(missing)}. "
            "Place operator-sourced wa_tribal_profiles.json — "
...
def load_wa_tribal_profiles(*, require_files: bool = True) -> list[dict[str, Any]]:
    if require_files:
    ...
    if isinstance(raw, list):
        ...
        raise ValueError(
            f"wa_tribal_profiles.json: expected {PROFILES_SCHEMA!r} object or "
            f"research profile array — got {type(raw).__name__}"
        )
    if len(profiles) != EXPECTED_PROFILE_COUNT:
        raise ValueError(
            f"expected {EXPECTED_PROFILE_COUNT} WA tribal profiles, got {len(profiles)}"
        )
    return _json_safe(profiles)


def build_wa_pilot_fixture_contract() -> dict[str, Any]:
    present = fixtures_present()
    ready = all(present.values())
    profile_count = 0
    if ready:
        profile_count = len(load_wa_tribal_profiles(require_files=False))
    return _json_safe(
        {
            "schema_version": SCHEMA_VERSION,
            "fixtures_dir": str(_WA_PILOT_DIR),

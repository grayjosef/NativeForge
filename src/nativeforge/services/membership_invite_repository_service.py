...
def record_acceptance(
    *,
    ...
    """Mark an invite accepted by an identity that exists and is not its author.

    This is the step nothing here can supply on its own. An accepter must be a
    real `nf_identities` row, and must not be the requester or the approver.
    """
    organization = _as_uuid(organization_id)
    accepter = _as_uuid(accepted_by_identity_id)
    blocked: list[str] = []
    ...
    if connection is not None and accepter is not None:
        exists = connection.execute(
            sa.text(f"SELECT COUNT(*) FROM {IDENTITY_TABLE} WHERE id = :i"),
            {"i": accepter.hex},
        ).scalar_one()
        if not exists:
...
def build_invite_binding_evidence(*, connection: Any = None) -> dict[str, Any]:
    ...
    try:
        ...
        accepted_rows = (
            connection.execute(
                sa.select(INVITES.c.accepted_by_identity_id).where(
    ...
    try:
        membership_rows = (
            connection.execute(
                sa.text(
                    f"SELECT identity_id FROM {MEMBERSHIP_TABLE} "

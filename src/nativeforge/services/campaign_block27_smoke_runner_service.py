...
SCHEMA_VERSION = "nf_campaign_block27_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block27_smoke")


def run_campaign_block27_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp27_national_coverage_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_national_coverage_demo_surface()
    fails.extend(national_coverage_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("national_coverage"):
        fails.append("bridge_missing_national_coverage")
    ...
    result = {
        ...
        "campaign_block": 27,
        "fails": fails,
        "top_15_count": surface.get("top_15_count"),

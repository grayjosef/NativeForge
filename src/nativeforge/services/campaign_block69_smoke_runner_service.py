...
SCHEMA_VERSION = "nf_campaign_block69_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block69_smoke")


def run_campaign_block69_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp69_onboarding_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    surface = build_pilot_onboarding_demo_surface()
    fails.extend(pilot_onboarding_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("pilot_org_onboarding"):
        fails.append("bridge_missing_pilot_org_onboarding")

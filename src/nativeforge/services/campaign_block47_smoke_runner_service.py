...
SCHEMA_VERSION = "nf_campaign_block47_smoke_v1"
DEFAULT_OUT = Path("artifacts/campaign_block47_smoke")


def run_campaign_block47_smoke() -> dict[str, Any]:
    run_id = (
        f"nf_camp47_auth0_unlock_smoke_"
        f"{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    )
    fails: list[str] = []
    attempt = run_auth0_mode_b_live_unlock_attempt()
    if attempt.get("mode_detected") != "mode_a":
        fails.append("expected_mode_a")
    if attempt.get("login_live_claimed"):
        fails.append("login_live")
    if attempt.get("secret_value_printed"):
        fails.append("secret_printed")
    surface = build_auth0_mode_b_live_unlock_demo_surface()
    fails.extend(auth0_mode_b_live_unlock_demo_surface_invariant_failures(surface))
    bridge = build_sc_customer_demo_bridge_payload()
    fails.extend(bridge_payload_invariant_failures(bridge))
    if not bridge.get("auth0_mode_b_live_unlock"):
        fails.append("bridge_missing_auth0_mode_b_live_unlock")

"""WA-3: load WA pilot profiles through RT-1 provenance bridge (public_inferred)."""
...
PROFILE_WA_PILOT_PREFIX = "wa_pilot_"
...
def list_wa_pilot_profiles(*, require_files: bool = False) -> list[dict[str, Any]]:
    rows = load_wa_tribal_profiles(require_files=require_files)
    return [
        {
...
def resolve_wa_pilot_profile(
    ...
    *,
    ...
    if require_files:
        require_wa_pilot_fixtures()
    for raw in load_wa_tribal_profiles(require_files=require_files):
        if str(raw.get("fixture_key")) == fixture_key:
            ...
            return profile
    raise ValueError(f"unknown WA pilot profile fixture_key: {fixture_key!r}")


def build_wa_pilot_profile_contract() -> dict[str, Any]:
    from nativeforge.services.wa_pilot_fixture_loader_service import (
    ...
    return _json_safe(
        {
            ...
            "fixture_prefix": PROFILE_WA_PILOT_PREFIX,
            "capture_method": OAP_PUBLIC_INFERRED,
            "fixtures": build_wa_pilot_fixture_contract(),
            "profiles": list_wa_pilot_profiles(require_files=False),
        }
    )

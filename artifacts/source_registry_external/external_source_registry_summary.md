...
**13 of 55 sources carry an obligation or a blocker.** None may be automated before that is resolved, and 3 may only ever be checked by a person.

## Capability is not approval

5 sources expose an API. **0 are approved for automated use.** The two are separate fields and an invariant keeps the approved count at zero until somebody clears them.

## State scoping

10 sources are state-scoped, covering SC. They are visible only to customers whose declared **operating state(s)** include that state - not their mailing address. A customer with no declared operating state sees none of them.
...
**3 sources reach the watchlist.** `sometimes_allowable` is deliberately excluded from it: most of the registry reads that way, so a watchlist including it would be the registry with extra steps.
...
23 cells read `UNKNOWN` and are carried through as written. An unknown capability is not an absent one.
